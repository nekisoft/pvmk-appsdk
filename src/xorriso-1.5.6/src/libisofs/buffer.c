...
struct iso_ring_buffer
{
    ...
    //p thread_mutex_t mutex;
    //p thread_cond_t empty;
    //p thread_cond_t full;
};
...
int iso_ring_buffer_new(size_t size, IsoRingBuffer **rbuf)
{
    ...
    //p thread_mutex_init(&buffer->mutex, NULL);
    //p thread_cond_init(&buffer->empty, NULL);
    //p thread_cond_init(&buffer->full, NULL);
...
void iso_ring_buffer_free(IsoRingBuffer *buf)
{
    ...
    free(buf->buf);
    //p thread_mutex_destroy(&buf->mutex);
    //p thread_cond_destroy(&buf->empty);
    //p thread_cond_destroy(&buf->full);
    free(buf);
}
    ...
    while (bytes_write < count) {
        ...
        while (buf->size == buf->cap) {
            ...
            if (buf->rend) {
                ...
                //p thread_mutex_unlock(&buf->mutex);
                return 0;
            }
            buf->times_full++;
            /* wait until space available */
            //p thread_cond_wait(&buf->full, &buf->mutex);
        }
        ...
        /* wake up reader */
        //p thread_cond_signal(&buf->empty);
        //p thread_mutex_unlock(&buf->mutex);
    }
    return ISO_SUCCESS;
}
...
int iso_ring_buffer_read(IsoRingBuffer *buf, uint8_t *dest, size_t count)
{
    ...
    while (bytes_read < count) {
        ...
        while (buf->size == 0) {
            ...
            //p thread_cond_wait(&buf->empty, &buf->mutex);
        }
