...
#include <string.h>
#include <ctype.h>
//#include <p thread.h> //pvmk - no p thread
#include <errno.h>
#include <fcntl.h>
...
struct burn_drive *burn_drive_register(struct burn_drive *d)
{
	...
	//p thread_mutex_init(&drive_array[i].access_lock, NULL); //pvmk - no p thread
	if (drivetop < i)
		drivetop = i;
	...
	//p thread_mutex_init(&drive_array[drivetop + 1].access_lock, NULL);  //pvmk - no p thread
...
/* ts B00225 : these mutexes are unnecessary because "= 1" is atomar.
	p thread_mutex_lock(&d->access_lock);
*/
	if (!d->cancel) {
	...
	d->cancel = 1;
/*
	p thread_mutex_unlock(&d->access_lock);  //pvmk - no p thread
*/
}
...
int burn_drive_find_by_thread_pid(struct burn_drive **d, pid_t pid,
								 /*p thread_t tid*/ int tid)   //pvmk - no p thread
{
	int i;
...
*/
		...
		//    drive_array[i].thread_pid == pid &&
		    //p thread_equal(drive_array[i].thread_tid, tid)) {
			//*d = &(drive_array[i]);
			//return 1;
		//}  //pvmk - no p thread
	}
