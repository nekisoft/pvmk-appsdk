...
void move_along_velocity_surface( Vehicle *v )
{
    ...
    len = sqrt( ( vel_temp[X] * vel_temp[X] ) + ( vel_temp[Y] * vel_temp[Y] ) );

    front_len = sqrt( ( v->orient.front[X] * v->orient.front[X] ) +
...
{
    float factor = (float)value / 255.0;
...
int old_push_vehicle_toward_surface( Vehicle *v )
{
    ...
    const float inc_landing_speed = 0.067;
    const float max_landing_speed = 0.35;
    ...
    len = sqrt( (v->orient.position[X] * v->orient.position[X]) + (v->orient.position[Y] * v->orient.position[Y]) );
...
int push_vehicle_toward_surface( Vehicle *v )
{
    ...
    if( (v->orient.position[X] < epsilon) && (v->orient.position[X] > -epsilon) &&
        ...
        len = sqrt( (v->orient.position[X] * v->orient.position[X]) +
...
void bounce_vehicle( Vehicle *v, Float_Vector reflect )
{
    ...
    const float surface_min_vel = 0.15;
    const float air_min_vel = 0.2;
    const float surface_elasticity = 0.65;
    const float air_elasticity = 0.65;
    float elasticity;
    float min_vel;
    ...
    if( (myvector_length( v->vel ) * elasticity) > min_vel ) {
        ...
        if( vector_dot_product( projection, reflect ) < 0 ) {
            bounce_vector[X] = (-2.0 * projection[X]) + v->vel[X];
            bounce_vector[Y] = (-2.0 * projection[Y]) + v->vel[Y];
            bounce_vector[Z] = (-2.0 * projection[Z]) + v->vel[Z];
        }
        else {
