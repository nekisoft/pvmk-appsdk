...
extern sb_sample *sample[MAX_WORLD_SAMPLES]; /* From omega.c */
...
void init_red_radar_base( RadarBase *red_base )
{
    ...
    if( ( red_base->base_obj = (PointFace *)malloc( sizeof( PointFace ) ) ) == NULL ) {
        printf("red_radar_base_init() : malloc failed\n");
        exit_gracefully();
    }
...
void init_blue_radar_base( RadarBase *blue_base )
{
    ...
    if( ( blue_base->base_obj = (PointFace *)malloc( sizeof( PointFace ) ) ) == NULL ) {
        printf("blue_radar_base_init() : malloc failed\n");
        exit_gracefully();
    }
...
void fire_radar_base( RadarBase *base, PointFace *obj )
{
    ...
    switch (base->laser_damage)
        {
         ...
         case 10 :
             ...
             break;
         default :
             printf("fire_radar_base() : base->laser_damage out of range!!\n");
             Get_Keypress();
             exit_gracefully();
             break;
        } /* End switch */
    ...
    if( count < base->max_projectiles ) {
        ...
        if( ( p->obj = (PointFace *)malloc( sizeof( PointFace ) ) ) == NULL ) {
            printf("fire_radar_base() : malloc failed\n");
            Kill_Keys();
            exit(1);
        }
        ...
        if( p->type == LASER ) {
            ...
            p->orient.position[X] = ftom( base->turret_orient.position[X] ) + ftom(0.125 * left[X]);

            p->orient.position[Y] = ftom( base->turret_orient.position[Y] ) + ftom(0.125 * left[Y]);

            p->orient.position[Z] = ftom( base->turret_orient.position[Z] ) + ftom(0.125 * left[Z]);
        ...
        */
        ...
        if( ( p->world_obj = (PointFace *)malloc( sizeof( PointFace ) ) ) == NULL ) {
            printf("fire_radar_base() : malloc_failed\n");
            Kill_Keys();
            exit(1);
        }
...
void aim_radar_base( RadarBase *base, Player *player, PointFace *obj )
{
    ...
    if( base->alive ) {
        ...
        for( i = 0; i < 6; i++ ) {
            ...
            len = sqrt( vector_dot_product( temp, temp ) );
