...
void SN76489_Init(int which, int PSGClockValue, int SamplingRate)
{
    ...
    p->dClock32b32=(1ull<<32) * PSGClockValue/16/SamplingRate;
...
void SN76489_Update(int which, INT16 **buffer, int length)
{
    ...
    for(j = 0; j < length; j++)
    {
        ...
        p->Clock32b32+=p->dClock32b32;
        p->NumClocksForSample=(int)(p->Clock32b32 >> 32);  /* truncates */
        p->Clock32b32-=((unsigned long long)p->NumClocksForSample)<<32;  /* remove integer part */
        ...
        for (i=0;i<=2;++i) {
            if (p->ToneFreqVals[i]<=0) {   /* If it gets below 0... */
                if (p->Registers[i*2]>PSG_CUTOFF) {
                    ...
                    p->IntermediatePos[i]=(long)((p->NumClocksForSample-(p->Clock32b32>>32)+2*p->ToneFreqVals[i])*p->ToneFreqPos[i]/(p->NumClocksForSample+(p->Clock32b32>>32))*65536);
