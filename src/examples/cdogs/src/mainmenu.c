...
{
  { "Bryan E. Topp",           "Nothing but the port to Neki32. I stole it from..." },
  { "Ronny Wester",            "That's me! I designed and coded this game and I did all the graphics too" },
  { "Joey Lau",                "My love. Although I had to alter the knife rating to survive the dogfights ;-)" },
  { "Jan-Olof Hendig",         "A friend with whom I've spent far too much time on the phone far too late" },
  { "Joakim Johansson",        "Friend and colleague. We keep each other sane...or is it the other way around?" },
  { "Victor Putz",             "Kindred spirit and a great guy to brainstorm with" },
  { "Ken Gorley",              "Found the 1st bug in the release: didn't work with a single joystick" },
  { "Adrian Stacey",           "Most helpful in tracking down the vsync bug" },
  { "Antti Hukkanen",		   "Helped locate a bug when using maximum number of characters in a mission" },
  ...
  { "Cyberdogs fans wherever", "Thanks! Without your support this program would never have come about" }
};
...
#ifndef SHAREWARE
  campaignList = GetFilesFromDirectory( "*.cpn");
...
#ifndef SHAREWARE
  dogfightList = GetFilesFromDirectory( "*.df");
...
int SelectCampaign( int dogFight, int cmd )
{
  ...
  if (AnyButton( cmd))
  {
    ...
    if (f && f->name[0])
    {
      ...
      if (LoadCampaign( f->name, &customSetting, 0, 0) == CAMPAIGN_OK)
...
int SelectControls( int cmd )
{
  ...
  TextStringAt( 230, 50 + 2*TextHeight(), gOptions.swapButtonsJoy1 ? "Yes" : "No");
...
void ChangeKey( struct PlayerData *data, struct PlayerData *other, int index )
{
  int key = 0;
...
static void DisplayKeys( int x, int x2, int y, char *title, struct PlayerData *data, int index, int change )
{
  ...
  for (i = 0; i < 6; i++)
    if (change == i)
      DisplayMenuItem( x2, y + (i+1)*TextHeight(), SELECTKEY, index == i);
    else
...
int SelectVolume( int cmd )
{
  ...
  if (Left( cmd))
  {
    switch (index)
    {
      ...
      case 2:
        ...
        break;
      case 3:
        SetDynamicInterrupts( !DynamicInterrupts());
        break;
    }
    PlaySound( SND_SWITCH, 0, 255);
  }
  ...
  TextStringAt( 230, 50 + 3*TextHeight(), DynamicInterrupts() ? "Yes" : "No");
...
static void ShowCredits( void )
{
  ...
  {
    creditIndex++;
    if ((unsigned)creditIndex >= sizeof( credits)/sizeof( credits[0]))
      creditIndex = 0;
    lastTick = t;
  }
}
...
void LoadConfig( void )
{
  ...
  if (f)
  {
    ...
    fscanf( f, "%d\n", &dynamic);
    SetDynamicInterrupts( dynamic);
    fscanf( f, "%s\n", s);
    SetModuleDirectory( s);
    fscanf( f, "%d\n", &gCampaign.seed);
    fscanf( f, "%d %d\n", &gOptions.difficulty, &gOptions.slowmotion);
