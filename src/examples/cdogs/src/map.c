...
void GuessCoords( int *x, int *y )
{
  if (gMission.missionData->mapWidth)
    *x = (rand() % gMission.missionData->mapWidth) + (XMAX - gMission.missionData->mapWidth)/2;
  else
    *x = rand() % XMAX;

  if (gMission.missionData->mapHeight)
    *y = (rand() % gMission.missionData->mapHeight) + (YMAX - gMission.missionData->mapHeight)/2;
  else
    *y = rand() % YMAX;
}
...
{
  if (gMission.missionData->mapWidth)
    *x = (rand() % (gMission.missionData->mapWidth * TILE_WIDTH)) +
        (XMAX - gMission.missionData->mapWidth)*TILE_WIDTH/2;
  else
    *x = rand() % (XMAX * TILE_WIDTH);

  if (gMission.missionData->mapHeight)
    *y = (rand() % (gMission.missionData->mapHeight * TILE_HEIGHT)) +
        (YMAX - gMission.missionData->mapHeight)*TILE_HEIGHT/2;
  else
...
void MakeRoom( int xOrigin, int yOrigin, int width, int height, int doors, int access )
{
  int x, y;
      ...
      switch (iMap( x, y) & MAP_MASKACCESS)
      {
        ...
        case MAP_DOOR:
          if (y > 0 && (Map( x, y - 1).flags & NO_SEE) != 0)
            ...
            Map( x, y).pic = cRoomPics[ room][ ROOMFLOOR_NORMAL];
          break;

        case MAP_WALL:
          Map( x, y).pic = cWallPics[ wall][ GetWallPic( x, y)];
          Map( x, y).flags = NO_WALK | NO_SEE | IS_WALL;
          break;
...
static int OneWall( int x, int y )
{
  ...
  if (x > 0 && y > 0 && x < XMAX-1 && y < YMAX-1)
  {
    if ((Map( x-1, y).flags & NO_WALK) != 0) count++;
    if ((Map( x+1, y).flags & NO_WALK) != 0) count++;
    if ((Map( x, y-1).flags & NO_WALK) != 0) count++;
    if ((Map( x, y+1).flags & NO_WALK) != 0) count++;
  }
  return (count == 1);
}
...
static int NoWalls( int x, int y )
{
  if (x > 0 && y > 0 && x < XMAX-1 && y < YMAX-1)
  {
    if ((Map( x-1, y).flags & NO_WALK) != 0) return 0;
    if ((Map( x+1, y).flags & NO_WALK) != 0) return 0;
    if ((Map( x, y-1).flags & NO_WALK) != 0) return 0;
    if ((Map( x, y+1).flags & NO_WALK) != 0) return 0;
    if ((Map( x-1, y-1).flags & NO_WALK) != 0) return 0;
    if ((Map( x+1, y+1).flags & NO_WALK) != 0) return 0;
    if ((Map( x+1, y-1).flags & NO_WALK) != 0) return 0;
    if ((Map( x-1, y+1).flags & NO_WALK) != 0) return 0;
    return 1;
...
static int PlaceOneObject( int x, int y, TMapObject *mo, int extraFlags )
{
  ...
  if ((f & MAPOBJ_ONEWALL) != 0 &&
      !OneWall( x, y))
    return 0;

  if ((f & MAPOBJ_ONEWALLPLUS) != 0 &&
      !OneWallOrMore( x, y))
    return 0;

  if ((f & MAPOBJ_NOWALLS) != 0 &&
      !NoWalls( x, y))
    return 0;
...
{
  int access = (gMission.missionData->objectives[ objective].flags & OBJECTIVE_HIACCESS) != 0 &&
               accessCount > 1;
  int noaccess = (gMission.missionData->objectives[ objective].flags & OBJECTIVE_NOACCESS) != 0;
  int x, y;
  ...
  while (i)
  {
    ...
    {
      if ((!access || (iMap( x/TILE_WIDTH, y/TILE_HEIGHT) & MAP_ACCESSBITS) != 0) &&
      	  (!noaccess || (iMap( x/TILE_WIDTH, y/TILE_HEIGHT) & MAP_ACCESSBITS) == 0))
      {
        AddObject( x << 8, y << 8, 3, 2,
                   &cGeneralPics[ gMission.objectives[ objective].pickupItem],
                   OBJ_JEWEL,
  ...
  while (i > 0)
  {
    	...
    	(!noaccess || (iMap( x, y) >> 8) == 0))
    {
      if (PlaceOneObject( x, y,
                          gMission.objectives[ objective].blowupObject,
  ...
  while (1)
  {
        ...
        Map( x, y+1).things == NULL)
    {
      AddObject( (x*TILE_WIDTH + TILE_WIDTH/2) << 8,
                 ...
                 9, 5,
                 &cGeneralPics[ gMission.keyPics[ pic]], card,
...
static void VertDoor( int x, int y, int flags )
{
  ...
  a[4].x = x*TILE_WIDTH; a[4].y = y*TILE_HEIGHT;
...
static void FixDoors( int floor, int room )
{
  ...
  for (x = 0; x < XMAX; x++)
    for (y = 0; y < YMAX; y++)
      ...
      {
        CreateDoor( x, y, floor, room, Access( x, y));
      }
}
...
void SetupMap( void )
{
  ...
  w = mission->mapWidth && mission->mapWidth < XMAX ? mission->mapWidth : XMAX;
  h = mission->mapHeight && mission->mapHeight < YMAX ? mission->mapHeight : YMAX;
  x = (XMAX - w)/2;
  y = (YMAX - h)/2;
  tilesTotal = w * h;
  ...
  for (i = 0; i < gMission.objectCount; i++)
    for (j = 0; j < (mission->itemDensity[i] * tilesTotal)/1000; j++)
      PlaceObject( x + rand() % w, y + rand() % h, i);

  for (i = 0, j = 0; i < mission->objectiveCount; i++)
    if (mission->objectives[i].type == OBJECTIVE_COLLECT)
    {
      for (j = 0, count = 0; j < gMission.objectives[i].count; j++)
        if (PlaceCollectible( i))
          count++;
      gMission.objectives[i].count = count;
      if (gMission.objectives[i].count < gMission.objectives[i].required)
        gMission.objectives[i].required = gMission.objectives[i].count;
    }
