...
void SetBuffer( int x_origin, int y_origin, struct Buffer *buffer, int width )
{
  ...
  {
    for (x = buffer->xStart; x < buffer->xStart + buffer->width; x++, bufTile++)
...
void FixBuffer( struct Buffer *buffer, int isShadow )
{
  ...
  for (y = 0; y < Y_TILES; y++)
  {
    for (x = 0; x < buffer->width; x++, tile++)
    {
      ...
      {
        tile->things = NULL;
        if ((tile->flags & (IS_WALL | DELAY_DRAW)) == IS_WALL)
...
void DrawWallColumn( int y, int xc, int yc, TTile *tile )
{
  ...
  {
    DrawPic( xc, yc, gPics[ tile->pic], gCompiledPics[ tile->pic]);
    yc -= TILE_HEIGHT;
    tile -= X_TILES;
    y--;
  }
...
void DrawBuffer( struct Buffer *b, int xOffset )
{
  ...
  {
    for (x = 0, xc = b->dx + xOffset; x < b->width; x++, tile++, xc += TILE_WIDTH)
    {
      if (tile->pic >= 0 &&
  ...
  for (y = 0; y < Y_TILES; y++, yc += TILE_HEIGHT)
  {
    ...
    for (x = 0; x < b->width; x++, tile++, xc += TILE_WIDTH)
    {
      ...
      else if ((tile->flags & OFFSET_PIC) != 0)
      {
        ...
        DrawPic( xc + p->dx, yc + p->dy, gPics[ p->picIndex], gCompiledPics[ p->picIndex]);
      }
      t = tile->things;
      while (t)
      {
