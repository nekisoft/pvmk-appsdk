...
void DrawCharacter( int x, int y, TActor *actor )
{
  ...
  TranslationTable *table = &(c->table);
  int f = c->facePic;
  int b;
  int g = gunDesc[ actor->gun].gunPic;
  int gunState;

  TOffsetPic body = {0}, head = {0}, gun = {0};
  TOffsetPic pic1 = {0}, pic2 = {0}, pic3 = {0};
  ...
  if (actor->dead)
  {
    if (actor->dead <= DEATH_MAX)
    {
      ...
      if (transparent)
        DrawBTPic( x + body.dx, y + body.dy, gPics[ body.picIndex], table, gRLEPics[ body.picIndex]);
      else
  ...
  if (g >= 0)
  {
    gun.dx = cGunHandOffset[ b][ dir].dx + cGunPics[ g][ dir][ gunState].dx;
    gun.dy = cGunHandOffset[ b][ dir].dy + cGunPics[ g][ dir][ gunState].dy;
  ...
  {
    if (pic1.picIndex >= 0)
      DrawBTPic( x + pic1.dx, y + pic1.dy, gPics[ pic1.picIndex], table, gRLEPics[ pic1.picIndex]);
    if (pic2.picIndex >= 0)
      DrawBTPic( x + pic2.dx, y + pic2.dy, gPics[ pic2.picIndex], table, gRLEPics[ pic2.picIndex]);
    if (pic3.picIndex >= 0)
      DrawBTPic( x + pic3.dx, y + pic3.dy, gPics[ pic3.picIndex], table, gRLEPics[ pic3.picIndex]);
  }
  else if (table)
  {
    if (pic1.picIndex >= 0)
      DrawTTPic( x + pic1.dx, y + pic1.dy, gPics[ pic1.picIndex], table, gRLEPics[ pic1.picIndex]);
    if (pic2.picIndex >= 0)
      DrawTTPic( x + pic2.dx, y + pic2.dy, gPics[ pic2.picIndex], table, gRLEPics[ pic2.picIndex]);
    if (pic3.picIndex >= 0)
      DrawTTPic( x + pic3.dx, y + pic3.dy, gPics[ pic3.picIndex], table, gRLEPics[ pic3.picIndex]);
  }
  else
  {
    if (pic1.picIndex >= 0)
...
void UpdateActorState( TActor *actor, int ticks )
{
  ...
  if (actor->state == STATE_IDLE && !actor->petrified)
    SetStateForActor( actor, ((rand()&1) != 0 ?
                             STATE_IDLELEFT : STATE_IDLERIGHT));
  else
...
int MoveActor( TActor *actor, int x, int y )
{
  ...
  {
    if (CheckWall( actor->x, y, actor->tileItem.w, actor->tileItem.h))
      y = actor->y;
    if (CheckWall( x, actor->y, actor->tileItem.w, actor->tileItem.h))
      x = actor->x;
  ...
  target = CheckTileItemCollision( &actor->tileItem, x >> 8, y >> 8, TILEITEM_IMPASSABLE);
  if (target)
  {
    if ((actor->flags & FLAGS_PLAYERS) != 0 && target->kind == KIND_CHARACTER)
    {
      otherCharacter = target->data;
      if (otherCharacter && (otherCharacter->flags & FLAGS_PRISONER) != 0)
      {
        otherCharacter->flags &= ~FLAGS_PRISONER;
        CheckMissionObjective( otherCharacter->tileItem.flags);
      }
    }
    ...
    if (CheckTileItemCollision( &actor->tileItem, actor->x >> 8, y >> 8, TILEITEM_IMPASSABLE))
      y = actor->y;
    if (CheckTileItemCollision( &actor->tileItem, x >> 8, actor->y >> 8, TILEITEM_IMPASSABLE))
      x = actor->x;
    if ((x == actor->x && y == actor->y) ||
        (x != actor->x && y != actor->y) ||
  ...
  {
    target = CheckTileItemCollision( &actor->tileItem, x >> 8, y >> 8, TILEITEM_CAN_BE_TAKEN);
...
void InjureActor( TActor *actor, int injury )
{
  ...
  {
    actor->stateCounter = 0;
    PlaySoundAt( actor->tileItem.x, actor->tileItem.y, screamTable[ scream]);
    scream++;
...
void LaunchGrenade( TActor *actor )
{
  ...
  AddGrenade( actor->x, actor->y, angle, actor->flags, MOBOBJ_GRENADE);
  actor->gunLock = 30;
...
void LaunchFragGrenade( TActor *actor )
{
  ...
  angle = dir2angle[ actor->direction];
  AddGrenade( actor->x, actor->y, angle, actor->flags, MOBOBJ_FRAGGRENADE);
  actor->gunLock = 30;
  Score( actor->flags, -20);
  PlaySoundAt( actor->tileItem.x, actor->tileItem.y, SND_LAUNCH);
}
...
void LaunchMolotov( TActor *actor )
{
  ...
  angle = dir2angle[ actor->direction];
  AddGrenade( actor->x, actor->y, angle, actor->flags, MOBOBJ_MOLOTOV);
  actor->gunLock = 30;
  Score( actor->flags, -20);
  PlaySoundAt( actor->tileItem.x, actor->tileItem.y, SND_LAUNCH);
}
...
void LaunchGasBomb( TActor *actor )
{
  ...
  angle = dir2angle[ actor->direction];
  AddGrenade( actor->x, actor->y, angle, actor->flags, MOBOBJ_GASBOMB);
  actor->gunLock = 30;
  Score( actor->flags, -10);
  PlaySoundAt( actor->tileItem.x, actor->tileItem.y, SND_LAUNCH);
}
...
void Flamer( TActor *actor )
{
  ...
  angle = dir2angle[ d];
  GetMuzzle( actor, &dx, &dy);
  AddFlame( actor->x + 256*dx, actor->y + 256*dy, angle, actor->flags);
  actor->gunLock = 6;
  Score( actor->flags, -1);
  if (!actor->sndLock)
  {
    PlaySoundAt( actor->tileItem.x, actor->tileItem.y, SND_FLAMER);
    actor->sndLock = 48;
  }
}
...
void PowerGun( TActor *actor )
{
  ...
  GetMuzzle( actor, &dx, &dy);
  AddLaserBolt( actor->x + 256*dx, actor->y + 256*dy, d, actor->flags);
  actor->gunLock = 20;
  Score( actor->flags, -2);
...
void SniperGun( TActor *actor )
{
  ...
  GetMuzzle( actor, &dx, &dy);
  AddSniperBullet( actor->x + 256*dx, actor->y + 256*dy, d, actor->flags);
  actor->gunLock = 100;
  Score( actor->flags, -5);
...
void ConfuseBomb( TActor *actor )
{
  ...
  angle = dir2angle[ actor->direction];
  AddGrenade( actor->x, actor->y, angle, actor->flags, MOBOBJ_GASBOMB2);
  actor->gunLock = 30;
  Score( actor->flags, -10);
  PlaySoundAt( actor->tileItem.x, actor->tileItem.y, SND_LAUNCH);
}
...
void CommandActor( TActor *actor, int cmd )
{
  ...
  if (actor->health > 0)
  {
    ...
    else if (!actor->petrified && (cmd & (CMD_LEFT | CMD_RIGHT | CMD_UP | CMD_DOWN)) != 0)
    {
      shallMove = YES;
...
void BuildTranslationTables( void )
{
  ...
  for (i = 0; i < 256; i++)
  {
    f = 0.3*gPalette[i].red + 0.59*gPalette[i].green + 0.11*gPalette[i].blue;
    tableFlamed[i] = BestMatch( f, 0, 0);
  }
  for (i = 0; i < 256; i++)
  {
    f = 0.4*gPalette[i].red + 0.49*gPalette[i].green + 0.11*gPalette[i].blue;
