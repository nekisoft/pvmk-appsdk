...
void DrawObject( int x, int y, const void *objv )
{
  const TObject *obj = (const TObject *)objv;
...
void DrawBullet( int x, int y, const void *objv )
{
  const TOffsetPic *pic;
  const TMobileObject *obj = (const TMobileObject *)objv;
...
void DrawBrownBullet( int x, int y, const void *objv )
{
  const TOffsetPic *pic;
  const TMobileObject *obj = (const TMobileObject *)objv;
...
void DrawPetrifierBullet( int x, int y, const void *objv )
{
  const TOffsetPic *pic;
  const TMobileObject *obj = (const TMobileObject *)objv;
...
void DrawSeeker( int x, int y, const void *objv )
{
  const TOffsetPic *pic;
  const TMobileObject *obj = (const TMobileObject *)objv;
...
void DrawMine( int x, int y, const void *objv )
{
  const TOffsetPic *pic;
  const TMobileObject *obj = (const TMobileObject *)objv;
...
void DrawDynamite( int x, int y, const void *objv )
{
  const TOffsetPic *pic;
  const TMobileObject *obj = (const TMobileObject *)objv;
...
void DrawMolotov( int x, int y, const void *objv )
{
  const TOffsetPic *pic;
  const TMobileObject *obj = (const TMobileObject *)objv;
...
void DrawFlame( int x, int y, const void *objv )
{
  const TOffsetPic *pic;
  const TMobileObject *obj = (const TMobileObject *)objv;
...
void DrawLaserBolt( int x, int y, const void *objv )
{
  const TOffsetPic *pic;
  const TMobileObject *obj = (const TMobileObject *)objv;
...
void DrawBrightBolt( int x, int y, const void *objv )
{
  const TOffsetPic *pic;
  const TMobileObject *obj = (const TMobileObject *)objv;
...
void DrawSpark( int x, int y, const void *objv )
{
  const TOffsetPic *pic;
  const TMobileObject *obj = (const TMobileObject *)objv;
...
void DrawGrenade( int x, int y, const void *objv )
{
  const TOffsetPic *pic;
  const TMobileObject *obj = (const TMobileObject *)objv;
...
void DrawGasCloud( int x, int y, const void *objv )
{
  const TOffsetPic *pic;
  const TMobileObject *obj = (const TMobileObject *)objv;
...
void DrawFireball( int x, int y, const void *objv )
{
  const TOffsetPic *pic;
  const TMobileObject *obj = (const TMobileObject *)objv;
...
void BogusDraw( int x, int y, const void *data )
{
  //if (data) x = y; // Just to get rid of warning...
  (void)x; // this is the right way man! -betopp about 30 years later
  (void)y;
  (void)data;
}
...
void Gas( int x, int y, int flags, int special )
{
  ...
  flags |= FLAGS_HURTALWAYS;
  for (i = 0; i < 8; i++)
    AddGasCloud( x, y, rand() & 255, (256 + rand()) & 255, (48 - rand()%8)*4 - 1, //parens fucked? -betopp
                flags, special);
  PlaySoundAt( x >> 8, y >> 8, SND_BANG);
}
...
void InternalAddObject( int x, int y, int w, int h,
                        const TOffsetPic *pic, const TOffsetPic *wreckedPic, int structure,
