...
void R_Alias_clip_z (finalvert_t *pfv0, finalvert_t *pfv1, finalvert_t *out)
{
	...
	if (pfv0->v[1] >= pfv1->v[1])
	{
		...
		avout.fv[0] = pav0->fv[0] + rf_mul((pav1->fv[0] - pav0->fv[0]), scale);
		avout.fv[1] = pav0->fv[1] + rf_mul((pav1->fv[1] - pav0->fv[1]), scale);
		avout.fv[2] = ALIAS_Z_CLIP_PLANE;
	
		out->v[2] =	pfv0->v[2] + rf_mul((pfv1->v[2] - pfv0->v[2]), scale);
		out->v[3] =	pfv0->v[3] + rf_mul((pfv1->v[3] - pfv0->v[3]), scale);
		out->v[4] =	pfv0->v[4] + rf_mul((pfv1->v[4] - pfv0->v[4]), scale);
	}
	else
	{
		...
		avout.fv[0] = pav1->fv[0] + rf_mul((pav0->fv[0] - pav1->fv[0]), scale);
		avout.fv[1] = pav1->fv[1] + rf_mul((pav0->fv[1] - pav1->fv[1]), scale);
		avout.fv[2] = ALIAS_Z_CLIP_PLANE;
	
		out->v[2] =	pfv1->v[2] + rf_mul((pfv0->v[2] - pfv1->v[2]), scale);
		out->v[3] =	pfv1->v[3] + rf_mul((pfv0->v[3] - pfv1->v[3]), scale);
		out->v[4] =	pfv1->v[4] + rf_mul((pfv0->v[4] - pfv1->v[4]), scale);
	}
