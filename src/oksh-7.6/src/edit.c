...
plain_fmt_entry(void *arg, int i, char *buf, int bsize)
{
	...
	if (buf == NULL || bsize <= 0)
		internal_errorf("%s: buf %p, bsize %d",
		    __func__, (void*) buf, bsize);
    ...
    int *nwords, char ***words)
{
	...
	*words = NULL;
	...
	while (cmd < want && isspace((unsigned char)*cmd))
		cmd++;
	cmdlen = 0;
	while (cmd + cmdlen < want && !isspace((unsigned char)cmd[cmdlen]))
		cmdlen++;
	for (i = 0; i < cmdlen; i++) {
		if (!isalnum((unsigned char)cmd[i]) && cmd[i] != '_')
			return 0;
	}
	...
	for (cp = cmd + cmdlen + 1; cp < want; cp++) {
		if (!isspace((unsigned char)cp[-1]) && isspace((unsigned char)*cp))
			n++;
	}
