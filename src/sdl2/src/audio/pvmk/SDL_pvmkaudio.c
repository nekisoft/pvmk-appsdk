...
static void PVMKAUD_DspHook(DSP_HookType hook)
{
...
static int PVMKAUDIO_OpenDevice(_THIS, const char *devname)
{
    ...
    dspHook(&dsp_hook, PVMKAUD_DspHook);

    return 0;
}

static int PVMKAUDIO_CaptureFromDevice(_THIS, void *buffer, int buflen)
{
...
static void PVMKAUDIO_PlayDevice(_THIS)
{
    size_t nextbuf;
    size_t sampleLen;
...
static void PVMKAUDIO_WaitDevice(_THIS)
{
    contextLock(this);
...
static void PVMKAUDIO_ThreadInit(_THIS)
{
...
static SDL_bool PVMKAUDIO_Init(SDL_AudioDriverImpl *impl)
{
    ...
    impl->ThreadInit = PVMKAUDIO_ThreadInit;
    impl->OnlyHasDefaultOutputDevice = SDL_TRUE;

    /* Should be possible, but micInit would fail */
    impl->HasCaptureSupport = SDL_FALSE;
    impl->CaptureFromDevice = PVMKAUDIO_CaptureFromDevice;
